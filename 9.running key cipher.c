...
char encrypt_running_key_char(char plaintext_char, char key_char) {
...
void encrypt_running_key(char *plaintext, char *ciphertext, char *key) {
    ...
    for (int i = 0; i < plaintext_len; i++) {
        ...
        ciphertext[i] = encrypt_running_key_char(plaintext[i], current_key_char);
    }
    ciphertext[plaintext_len] = '\0';
}

char decrypt_running_key_char(char ciphertext_char, char key_char) {
...
void decrypt_running_key(char *ciphertext, char *plaintext, char *key) {
    ...
    for (int i = 0; i < ciphertext_len; i++) {
        ...
        plaintext[i] = decrypt_running_key_char(ciphertext[i], current_key_char);
    }
...
int main() {
    ...
    printf("Enter the initial key (alphabetic): ");
    fgets(key, sizeof(key), stdin);
    key[strcspn(key, "\n")] = 0;

    encrypt_running_key(plaintext, ciphertext, key);
    printf("Ciphertext: %s\n", ciphertext);

    decrypt_running_key(ciphertext, decrypted_plaintext, key);
